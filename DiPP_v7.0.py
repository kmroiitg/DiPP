...
class THzAnalysisGUI:
    def __init__(self, master):
        ...
        self.plot_frame.grid(row=0, column=9, rowspan=11, columnspan=4)  # Adjust grid position as needed
    ...
    def run_analysis(self):
        try:
            ...
            self.fft_ref = np.fft.rfft(y1)
            self.fft_sam = np.fft.rfft(y2)
    ...
    def display_plots(self):
        ...
        ax[1].plot(self.xf, np.abs(self.fft_ref)[:self.N1 // 2],label = "Reference")
        ax[1].plot(self.xf, np.abs(self.fft_sam)[:self.N1 // 2],label = "Sample")
        ax[1].legend()
        ax[1].set_xlabel('THz')
        ax[1].set_ylabel('FFT')
        ax[1].set_yscale('log')

        ax[2].plot(self.xf_sliced, self.trans, color='darkgray', linewidth=1, label='Exp')
        ax[2].plot(self.xf_sliced, self.trans_fit, label='w/p=' + str(self.w) + '/' + str(self.p))
        ax[2].set_xlabel('Frequency (THz)')
        ax[2].legend()
        ax[2].set_ylabel('Transmission')
        ax[2].set_xlim(self.freq1, self.freq2)
        ax[2].set_ylim(0, 1.0)
        plt.suptitle(Title)
        plt.tight_layout()
